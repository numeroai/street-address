...
class StreetAddressUsTest < Minitest::Test
  ...
  INFORMAL_ADDRESSES = {
    ...
    "123 Main St Unit 7sw" => { # no separator, trailing two-letter directional splits off
      :number => '123',
      :street => 'Main',
      :street_type => 'St',
      :unit_prefix => 'Unit',
  ...
  def compare_expected_to_actual_hash(expected, actual, address)
    expected.each_pair do |expected_key, expected_value|
      if expected_value.nil?
        assert_nil actual[expected_key], "For address '#{address}', expected nil but got #{actual[expected_key]}"
      else
        assert_equal expected_value, actual[expected_key], "For address '#{address}', #{expected_value} != #{actual[expected_key]}"
      end
    end
  end
